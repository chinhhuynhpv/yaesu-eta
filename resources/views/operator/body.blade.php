...
@section('body_class', 'admin-page operator-page')
...
@endauth
    ...
    <div class="header-center-box col-md-6 text-center">
        <div class="header-customerandshopname">
@if (! isset($users) )
...
@endif
@if (! isset($shops) )
    @if ( isset($shop) and $shop and isset($shop->name) )
            <span class="dsp-ib customername">&nbsp;&nbsp; / &nbsp;&nbsp;
            {{$shop->name}}       
            </span>
            <span class="dsp-ib sama">{{__('Shop Dono')}}</span>
    @endif    
@endif
        </div>
    </div>
