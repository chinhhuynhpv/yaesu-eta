...
@section('container')
    <div>
        <div class="row">
            <div class="col-md-10"><h3>{{__("Option Plan List")}}</h3></div>
            <div class="col-md-2">
        ...
        @if ($list->count() > 0)
            <table class="table table-borderless list" data-delete-url="{{route("{$prefixRouteName}HandleDelete")}}">
                ...
                <tr>
                    ...
                    <th scope="col">{{__("Option type")}}</th>
                    ...
                    <th scope="col">{{__("Discount target 1")}}</th>
                    <th scope="col">{{__("Discount target 2")}}</th>
                    <th scope="col">{{__("Discount target 3")}}</th>
                    <th scope="col">{{__("Discount target 4")}}</th>
                    <th scope="col">{{__("Discount target 5")}}</th>
                ...
                @foreach($list as $key => $item)
                    <tr>
                        ...
                        <td>{{$item->discount_target1}}</td>
                        <td>{{$item->discount_target2}}</td>
                        <td>{{$item->discount_target3}}</td>
                        <td>{{$item->discount_target4}}</td>
                        <td>{{$item->discount_target5}}</td>
                        <td>
                            <a href="{{route("{$prefixRouteName}Detail", ['id' => $item->id])}}">{{__("View")}}</a>
                            <a  href="" data-id="{{$item->id}}" data-name="{{$item->plan_num}}" data-btn-delete>{{__("Delete")}}</a>
                            <a href="{{route("{$prefixRouteName}Edit", ['id' => $item->id])}}">{{__("Update")}}</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
            ...
            {{ $list->links() }}
        @else
            <div class="mt-3">{{__('No option plan found.')}}</div>
        @endif
    </div>
